...
function AnchorBadge({ status }: { status?: string }) {
  const resolvedStatus = (status as AnchorStatus) ?? 'missing'
  const config = anchorConfig[resolvedStatus]
  const Icon = config.icon
  const colorMap: Record<AnchorStatus, string> = {
    recorded:
      'bg-emerald-50/60 text-emerald-600 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900',
    pending:
      'bg-amber-50/60 text-amber-600 border-amber-100 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900',
    missing:
      'bg-rose-50/60 text-rose-600 border-rose-100 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900',
    unavailable:
      'bg-slate-50/60 text-slate-500 border-slate-100 dark:bg-slate-900/20 dark:text-slate-400 dark:border-slate-800',
  }

  return (
    <span
      className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs ${colorMap[resolvedStatus]}`}
    >
...
>({
  ...
  render: ({ result, status }) => {
    if (status.type === 'running') {
      return <ToolLoading label='查询存证记录' />
    }
          ...
          {records.map((record, index) => (
            ...
            >
              ...
              <div className='flex items-center gap-1.5'>
                ...
                <AnchorBadge status={String(record.anchor_status)} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className='grid grid-cols-2 gap-x-6 gap-y-2 text-sm'>
          <div>
            ...
            <span className='ml-2 font-medium'>
              {result?.digest_match ? '是' : '否'}
            </span>
          </div>
          <div>
            <span className='text-muted-foreground'>回执状态</span>
            <span className='ml-2'>
              {String(result?.receipt_status ?? '未知')}
            </span>
          </div>
          <div>
            <span className='text-muted-foreground'>事实 ID</span>
            <span className='ml-2 font-mono text-xs'>
              {String(result?.fact_id ?? '未知')}
            </span>
          </div>
          <div>
            <span className='text-muted-foreground'>交易哈希</span>
            <span className='ml-2 font-mono text-xs'>
              {String(result?.tx_hash ?? '未知')}
            </span>
          </div>
        </div>
...
>({
  ...
  render: ({ result, status }) => {
    if (status.type === 'running') {
      return <ToolLoading label='追踪版本链路' />
    }
    ...
    return (
      <ToolCard
        icon={GitBranch}
        title={`版本链路 · ${result?.asset_id ?? ''}`}
        color='text-muted-foreground'
      >
        <div className='relative space-y-3 pl-4'>
          <div className='absolute top-1 bottom-1 left-[5px] w-px bg-border' />
          {versions.map((version, index) => (
            <div key={index} className='relative flex items-start gap-2.5'>
              <div className='absolute top-2 -left-2.5 h-2 w-2 rounded-full bg-emerald-600 ring-2 ring-background' />
        ...
        {matches.length === 0 ? (
          ...
          <div className='space-y-2'>
            {matches.map((match, index) => (
              ...
              >
                <p className='font-medium'>{String(match.filename)}</p>
                <p className='font-mono text-xs text-muted-foreground/50'>
                  {String(match.asset_id)} / {String(match.version_id)}
                </p>
                <StatusBadge status={String(match.status)} />
              </div>
            ))}
          </div>
        )}
...
>({
  ...
  render: ({ result, status }) => {
    if (status.type === 'running') {
      return <ToolLoading label='生成报告' />
    }

    return (
      <ToolCard
        icon={FileText}
        title={result?.title ?? '复核报告'}
        color='text-muted-foreground'
      >
