...
export function EvidenceConsole() {
  ...
  const [preserveResult, setPreserveResult] = useState<PreserveResult | null>(
    null
  )
  const [status, setStatus] = useState<UploadStatus>('idle')
  const [error, setError] = useState<string | null>(null)
  const [isDragging, setIsDragging] = useState(false)
  const [preservePanelOpen, setPreservePanelOpen] = useState(false)
  ...
  async function preserveFile() {
    ...
    try {
      ...
      const payload = (await response
        .json()
        .catch(() => ({}))) as PreserveResult & {
        error?: { message?: string }
      }

      if (!response.ok) {
        throw new Error(
          payload.error?.message || `Chronofact API 返回 ${response.status}`
        )
      }
    ...
    <ScrollArea className='h-full min-h-0 w-full overflow-x-hidden'>
      <div className='min-w-0 overflow-x-hidden p-5 pb-24'>
        ...
        <p className='mb-5 text-xs leading-5 text-muted-foreground/60'>
          选择要登记的项目文件，生成 SHA-256 指纹并提交存证记录。
        </p>
        ...
        <div>
          <ConsoleSection
            title='项目文件存证'
            description={
              selectedFile ? selectedFile.name : '上传文件并生成存证指纹'
            }
            icon={Blocks}
            open={preservePanelOpen}
            onOpenChange={setPreservePanelOpen}
          >
    ...
    <Collapsible open={open} onOpenChange={onOpenChange}>
      <div className='overflow-hidden rounded-xl border bg-background/70'>
        <CollapsibleTrigger asChild>
          <button
            type='button'
            className='flex w-full min-w-0 items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-accent/50'
          >
